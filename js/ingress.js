...
// Which signal(s) are currently selected — shared across sidebar, chart, and map tabs
let activeFields = new Set();
let activeField  = null;  

// ─── DATA PIPELINE ───────────────────────────────────────────────────────────
// ── push / pushGPS / haversineMeters / ingest ──
...
const GPS_TELEPORT_THRESHOLD_M = 1000;
...
function pushGPS(t, lat, lon) { //TODO: MAPPING NOT INTEGRATED WITH SUNBEAM
  ...
  if (gpsHistory.length && t <= gpsHistory[gpsHistory.length - 1].t) {
    console.warn(`[pushGPS] non-increasing timestamp: ${t} <= ${gpsHistory[gpsHistory.length - 1].t}, correcting to ${gpsHistory[gpsHistory.length - 1].t + 1}`);
    t = gpsHistory[gpsHistory.length - 1].t + 1;
  }
